...
## Verifying the published install
A second job, `verify-npm-install`, runs after `npm-publish` succeeds: it installs the just-published `bookie-mcp` version from the registry into a throwaway directory, creates a disposable Neon project, pushes the bundled Prisma schema to it, starts the server, and confirms it responds over stdio (via the MCP inspector CLI) before deleting the project — the same "no clone" flow documented in the README. It needs a repo secret `NEON_API_KEY` (Neon console → Account settings → API keys); `neonctl` picks it up automatically. **Not yet set** — until it is, this job will fail on every release (the npm publish itself is unaffected, since it's a separate job). If the job is ever interrupted before its cleanup step runs (cancelled run, job timeout), it can leak a `bookie-verify-*` Neon project — check `neonctl projects list` periodically.
